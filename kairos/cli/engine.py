...
Unlike ``KairosApplication.run_headless()`` — which mandates an MQTT broker — this engine
...
async def _collect_balances(hb: KairosApplication) -> Dict[str, Dict[str, float]]:
...
async def _format_status_text(hb: KairosApplication) -> Optional[str]:
...
async def _write_snapshot(hb: KairosApplication, name: str, *, running: bool) -> None:
...
async def _serve(hb: KairosApplication, name: str) -> None:
