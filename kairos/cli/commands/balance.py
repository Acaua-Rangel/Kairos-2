...
def _exchange_assets(connector: str, total: Dict[str, Decimal], available: Dict[str, Decimal],
    ...
    Mirrors ``KairosApplication.exchange_balances_extra_df``: zero balances are hidden; values
    come from the pre-fetched rate-oracle prices via ``find_rate``.
    """
    from kairos.connector.utils import combine_to_hb_trading_pair
    from kairos.core.rate_oracle.utils import find_rate
    assets: List[dict] = []
...
async def _attach_positions(ub, result: Dict[str, dict], timeout: float) -> None:
    ...
    from kairos.cli.commands._common import position_dict as _position_dict
    for ex in result:
