...
from kairos.client.config.fee_overrides_config_map import fee_overrides_config_map, init_fee_overrides_config
...
class ClientConfigAdapter:
    ...
    def _encrypt_secrets(self, conf_dict: Dict[str, Any]):
        # SecretStr fields round-trip as plain text in yml (no keystore) — just unwrap them so
        # they're YAML-serializable.
        for attr, value in conf_dict.items():
            if isinstance(value, SecretStr):
                conf_dict[attr] = value.get_secret_value()
...
async def copy_strategy_template(strategy: str) -> str:
    """
    Look up template `.yml` file for a particular strategy in `kairos/templates` and copy it to the `conf` folder.
...
def get_strategy_config_map(
    ...
    try:
        ...
        if config_cls is None:  # legacy
            ...
            strategy_module = __import__(f"kairos.strategy.{strategy}.{cm_key}",
                                         fromlist=[f"kairos.strategy.{strategy}"])
...
def get_strategy_starter_file(strategy: str) -> Callable:
    ...
    `kairos/strategy/{STRATEGY_NAME}/start.py` file.
    """
    if strategy is None:
        return lambda: None
    try:
        strategy_module = __import__(f"kairos.strategy.{strategy}.start",
                                     fromlist=[f"kairos.strategy.{strategy}"])
...
def get_strategy_pydantic_config_cls(strategy_name: str):
    ...
    try:
        ...
        pydantic_cm_path = root_path() / "kairos" / "strategy" / strategy_name / f"{pydantic_cm_pkg}.py"
        if pydantic_cm_path.exists():
            pydantic_cm_class_name = f"{''.join([s.capitalize() for s in strategy_name.split('_')])}ConfigMap"
            pydantic_cm_mod = __import__(f"kairos.strategy.{strategy_name}.{pydantic_cm_pkg}",
...
async def create_yml_files_legacy():
    """
    Copy `kairos_logs.yml` and `conf_global.yml` templates to the `conf` directory on start up
    """
    for fname in listdir(TEMPLATE_PATH):
        if "_TEMPLATE" in fname and CONF_POSTFIX not in fname:
            ...
            if conf_path.endswith("kairos_logs.yml"):
