...
import kairos.core.rate_oracle.utils as rate_oracle_utils
from kairos.client.config.config_data_types import BaseClientModel, ClientConfigEnum
from kairos.client.config.config_methods import using_exchange as using_exchange_pointer
from kairos.client.config.config_validators import validate_bool, validate_float
from kairos.client.settings import DEFAULT_LOG_FILE_PATH, AllConnectorSettings
from kairos.connector.connector_base import ConnectorBase
from kairos.connector.connector_metrics_collector import (
...
if TYPE_CHECKING:
    from kairos.core.trading_core import TradingCore
...
class CoinGeckoRateSourceMode(RateSourceModeBase):
    ...
    def validate_api_tier(cls, v: str):
        from kairos.data_feed.coin_gecko_data_feed.coin_gecko_constants import CoinGeckoAPITier
...
class ClientConfigMap(BaseClientModel):
    ...
    logger_override_whitelist: List = Field(
        default=["kairos.strategy.arbitrage", "kairos.strategy.cross_exchange_market_making", "conf"]
    )
