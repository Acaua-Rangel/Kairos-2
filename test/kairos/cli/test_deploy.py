...
from kairos.cli.commands.deploy import deploy, resolve_target
...
class DeployCommandTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        launch_patch = patch("kairos.cli.commands.start.launch",
    ...
    def test_existing_config_stdin_values_applied(self):
        path = self.dirs["v2-script"] / "conf_x.yml"
        path.write_text("a: 1\n")
        with patch("kairos.cli.commands._common.read_json_object_from_stdin",
                   return_value={"a": 7}):
    ...
    def test_strategy_name_creates_a_config_then_starts(self):
        created = {"file": "conf_pmm.yml", "type": "controller", "applied": "a, b", "ready": True,
                   "next": "hbot start"}
        out = io.StringIO()
        with patch("kairos.cli.commands.deploy.resolve_target",
