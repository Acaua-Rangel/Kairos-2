...
from kairos.cli import bot, strategy_configs as sc
from kairos.cli.commands import start as start_mod
...
class ReplaceRunningTest(unittest.TestCase):
    def test_no_pid_is_a_noop(self):
        with patch.object(bot, "read_pid", return_value=None), \
                patch("kairos.cli.commands.start.os.kill") as kill:
    ...
    def test_dead_pid_clears_state(self):
        with patch.object(bot, "read_pid", return_value=123), \
                ...
                patch("kairos.cli.commands.start.os.kill", side_effect=ProcessLookupError):
            start_mod._replace_running(timeout=1.0)
    ...
    def test_still_alive_at_deadline_fails_with_timeout_code(self):
        ...
        with patch.object(bot, "read_pid", return_value=123), \
                ...
                patch.object(bot, "clear_pid") as clear, \
                patch("kairos.cli.commands.start.os.kill"), \
                patch("kairos.cli.commands.start.time", fake_time):
...
class LaunchTest(unittest.TestCase):
    ...
    def test_replace_stops_the_running_bot_first(self):
        with patch.object(sc, "resolve_config_type", return_value="v2-script"), \
                patch.object(bot, "running", return_value=True), \
                patch("kairos.cli.commands.start._replace_running") as replace:
    ...
    def test_foreground_execs_the_engine_in_place(self):
        ...
        with patch.object(sc, "resolve_config_type", return_value="v1-strategy"), \
                ...
                patch("kairos.cli.commands.start.prefix_path", return_value=str(self.tmp)), \
                patch("kairos.cli.commands.start.os", fake_os):
...
class SpawnDetachedTest(unittest.TestCase):
    def setUp(self):
        ...
        self.stack.enter_context(patch("kairos.cli.commands.start.prefix_path", return_value=str(self.tmp)))
        self.fake_subprocess = MagicMock()
        self.proc = MagicMock(pid=4242)
        self.fake_subprocess.Popen.return_value = self.proc
        self.stack.enter_context(patch("kairos.cli.commands.start.subprocess", self.fake_subprocess))
        self.fake_time = MagicMock()
        self.stack.enter_context(patch("kairos.cli.commands.start.time", self.fake_time))
        self.cmd = [sys.executable, "-m", "kairos.cli.engine", "--name", "n"]
    ...
    def test_engine_exit_during_startup_fails_with_the_log_tail(self):
        ...
        with patch("kairos.cli.commands.start._log_tail", return_value="boom"):
            with self.assertRaises(typer.Exit) as ctx:
