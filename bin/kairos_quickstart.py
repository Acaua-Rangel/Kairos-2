...
from bin.kairos import UIStartListener, detect_available_port
...
async def quick_start(args: argparse.Namespace):
    ...
    # Shared boot (yml, basic logging, system configs, paper-trade, build app). Logging is
    # re-initialized later in run_application with the strategy file name. MQTT autostarts only headless.
    hb = await bootstrap_application(client_config_map,
...
def main():
    ...
    ev_loop.run_until_complete(quick_start(args))
